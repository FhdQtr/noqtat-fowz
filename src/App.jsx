...
import {
  QUIZ_EASY, QUIZ_MEDIUM, QUIZ_HARD, QUIZ_VERY_HARD, ALL_QUIZ,
  getAllQuizByDifficulty,
  FALLBACK_MATH, generateRandomMathQuestion, generateMathBatch,
  FALLBACK_WORDS,
  FALLBACK_RIDDLES,
  FALLBACK_FLAGS,
  FALLBACK_TRUEFALSE,
...
// ─── Data: Imported from questions.js ─────────────────────────────────────
// Alias for backward compatibility
const FALLBACK_QUIZ = ALL_QUIZ;
const FALLBACK_TRUEFALE = FALLBACK_TRUEFALSE;
...
// ─── Question History (prevents repeats — persists in localStorage) ────────
const _usedQTexts = (() => {
try {
const saved = localStorage.getItem("nf_used_questions");
...
try {
localStorage.setItem("nf_used_questions", JSON.stringify(Array.from(_usedQTexts)));
} catch {}
}
...
_usedQTexts.clear();
try { localStorage.removeItem("nf_used_questions"); } catch {}
}
...
return shuffle(getAllQuizByDifficulty(difficulty)).filter(q => !_usedQTexts.has(q.q)).slice(0, count).map((q) => ({ …q, type: "mcq", mode: "quiz" }));
}
}
...
if (gm === “speed”) {
// استخدام المولد العشوائي للرياضيات
const mathQs = generateMathBatch(perMode, “mixed”);
questions.push(…mathQs);
return;
}
const src = gm === “quiz” ? FALLBACK_QUIZ : gm === “riddles” ? FALLBACK_RIDDLES : FALLBACK_WORDS;
...
try {
  ...
  // Fallback: استخدم الأسئلة المحلية مقسمة حسب المستوى
  const easyQs = shuffle(QUIZ_EASY).slice(0, 20).map(q => ({ ...q, type: "mcq", mode: "quiz" }));
  const medQs = shuffle(QUIZ_MEDIUM).slice(0, 25).map(q => ({ ...q, type: "mcq", mode: "quiz" }));
  const hardQs = shuffle(QUIZ_HARD).slice(0, 25).map(q => ({ ...q, type: "mcq", mode: "quiz" }));
  const vhardQs = shuffle(QUIZ_VERY_HARD).slice(0, 30).map(q => ({ ...q, type: "mcq", mode: "quiz" }));
  const fallback = [...easyQs, ...medQs, ...hardQs, ...vhardQs].slice(0, 100);
