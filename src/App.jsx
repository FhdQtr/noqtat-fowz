...
1. ممنوع منعاً باتاً: أي محتوى جنسي أو إيحائي، أي محتوى سياسي، أي ذكر لإسرائيل (استخدم فلسطين فقط)، أي محتوى عنصري أو طائفي أو مذهبي
1. التصنيفات المسموحة: ثقافة عامة، علوم، جغرافيا، تاريخ إسلامي، قرآن وسنة، رياضيات، لغة عربية، رياضة، تقنية، طبيعة
1. الأسئلة تكون متنوعة وغير مكررة
1. مستوى الصعوبة متوسط (مناسب للجميع)
1. الإجابات تكون واضحة وصحيحة 100%

أجب فقط بـ JSON بدون أي نص إضافي أو backticks.`;

// ─── Question History (prevents repeats — persists in sessionStorage) ────────
const _usedQTexts = (() => {
try {
const saved = sessionStorage.getItem(“nf_used_questions”);
return new Set(saved ? JSON.parse(saved) : []);
} catch { return new Set(); }
})();

function _saveUsedQTexts() {
try {
sessionStorage.setItem(“nf_used_questions”, JSON.stringify(Array.from(_usedQTexts)));
} catch {}
}

function _trackQuestions(qs) {
qs.forEach((q) => { if (q.q) _usedQTexts.add(q.q); });
if (_usedQTexts.size > 500) {
const arr = Array.from(_usedQTexts);
arr.slice(0, 150).forEach((k) => _usedQTexts.delete(k));
}
_saveUsedQTexts();
}

// مسح السجل لتحدي المعرفة عند بدء جلسة جديدة طويلة
function _clearUsedQuestions() {
_usedQTexts.clear();
try { sessionStorage.removeItem(“nf_used_questions”); } catch {}
}
...
// Filter out flags and truefalse from AI generation (handled by fallback)
const aiModes = modes.filter((m) => m !== “flags” && m !== “truefalse”);
if (aiModes.length === 0) return null;
...
const requestedModes = aiModes.map((m) => modeDescriptions[m]).filter(Boolean).join(”\n”);
...
modes.forEach((gm) => {
if (gm === “flags”) {
const filtered = shuffle(FALLBACK_FLAGS).filter((q) => !usedTexts.has(q.q));
const pool = filtered.length >= perMode ? filtered : shuffle(FALLBACK_FLAGS);
questions.push(…pool.slice(0, perMode).map((q) => ({ …q, type: “flag”, mode: “flags” })));
return;
}
if (gm === “truefalse”) {
const filtered = shuffle(FALLBACK_TRUEFALE).filter((q) => !usedTexts.has(q.q));
const pool = filtered.length >= perMode ? filtered : shuffle(FALLBACK_TRUEFALE);
questions.push(…pool.slice(0, perMode).map((q) => ({ …q, type: “truefalse”, mode: “truefalse” })));
return;
}
...
// ─── Fallback: True/False Data ───────────────────────────────────────────────
const FALLBACK_TRUEFALE = [
...
{ q: “البشر والقردة العليا لهم نفس الأسلاف البعيدة”, answer: true },
];
...
{ id: “flags”, name: “تحدي الأعلام”, icon: “🚩”, desc: “اعرف علم أي دولة؟”, color: “#fbbf24” },
{ id: “truefalse”, name: “صح أو خطأ”, icon: “✅”, desc: “صح أم خطأ؟ فكّر سريع!”, color: “#ec4899” },
];
...
<style>{`@keyframes particleFloat { 0%, 100% { transform: translateY(0) translateX(0); opacity: 0.2; } 25% { transform: translateY(-30px) translateX(15px); opacity: 0.5; } 50% { transform: translateY(-15px) translateX(-10px); opacity: 0.3; } 75% { transform: translateY(-40px) translateX(20px); opacity: 0.4; } } @keyframes pulse { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } } @keyframes fadeIn { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } } @keyframes slideUp { from { opacity: 0; transform: translateY(40px); } to { opacity: 1; transform: translateY(0); } } @keyframes scaleIn { from { opacity: 0; transform: scale(0.8); } to { opacity: 1; transform: scale(1); } } @keyframes glow { 0%, 100% { box-shadow: 0 0 20px rgba(0,212,255,0.2); } 50% { box-shadow: 0 0 40px rgba(0,212,255,0.4); } } @keyframes shake { 0%,100% { transform: translateX(0); } 25% { transform: translateX(-5px); } 75% { transform: translateX(5px); } } @keyframes confetti { 0% { transform: translateY(0) rotate(0deg); opacity: 1; } 100% { transform: translateY(-200px) rotate(720deg); opacity: 0; } } @keyframes spin { to { transform: rotate(360deg); } } * { box-sizing: border-box; margin: 0; padding: 0; } input:focus { outline: none; } ::selection { background: ${theme.accent}44; } ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-track { background: ${theme.bg}; } ::-webkit-scrollbar-thumb { background: ${theme.border}; border-radius: 3px; }`}</style>
...
// level 1-10: easy
…Array.from({length: 10}, (*, i) => ({ level: i + 1, diff: “easy” })),
// level 11-20: medium
…Array.from({length: 10}, (*, i) => ({ level: i + 11, diff: “medium” })),
// level 21-30: hard
…Array.from({length: 10}, (*, i) => ({ level: i + 21, diff: “hard” })),
// level 31-50: veryhard
…Array.from({length: 20}, (*, i) => ({ level: i + 31, diff: “veryhard” })),
...
```
    ...
    {type === "flag" && (
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        <div style={{
          fontSize: 100,
          lineHeight: 1,
          filter: phase === "question" ? "none" : "none",
          animation: phase === "question" ? "pulse 2s ease-in-out infinite" : "none",
        }}>
          {question.flag}
        </div>
      </div>
  ...
  {/* Flag answers - same as mcq */}
  {phase === "question" && type === "flag" && (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
      {question.options.map((opt, i) => (
        <button
          key={i}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      <button
        ...
        style={{
          ...
          border: `2px solid ${theme.green}44`,
          borderRadius: 16,
          color: theme.green,
          fontSize: 22,
          fontWeight: 900,
        ...
        onMouseEnter={(e) => { e.currentTarget.style.background = `${theme.green}25`; e.currentTarget.style.borderColor = theme.green; }}
        onMouseLeave={(e) => { e.currentTarget.style.background = `${theme.green}15`; e.currentTarget.style.borderColor = `${theme.green}44`; }}
      >
      ...
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.background = `${theme.red}25`; e.currentTarget.style.borderColor = theme.red; }}
        onMouseLeave={(e) => { e.currentTarget.style.background = `${theme.red}15`; e.currentTarget.style.borderColor = `${theme.red}44`; }}
      >
        <span style={{ fontSize: 40 }}>❌</span>
        خطأ
      </button>
  ...
  {phase === "result" && (
    <div style={{ animation: "scaleIn 0.4s ease-out" }}>
      ...
      {/* Flag result */}
      {type === "flag" && (
        <Card hoverEffect={false} style={{ textAlign: "center", marginBottom: 20,
          background: players.find((p) => p.isHuman)?.correct ? `${theme.green}10` : `${theme.red}10`,
          borderColor: players.find((p) => p.isHuman)?.correct ? `${theme.green}44` : `${theme.red}44`,
        }}>
          <div style={{ fontSize: 72, marginBottom: 8 }}>{question.flag}</div>
          <div style={{ fontSize: 20, fontWeight: 800, fontFamily: "Cairo", marginBottom: 4,
            color: players.find((p) => p.isHuman)?.correct ? theme.green : theme.red }}>
            {players.find((p) => p.isHuman)?.correct ? "✅ صح!" : "❌ غلط!"}
          </div>
          <div style={{ color: theme.green, fontSize: 20, fontWeight: 800, fontFamily: "Cairo" }}>
      ...
      {type === "truefalse" && (
        <Card hoverEffect={false} style={{ textAlign: "center", marginBottom: 20,
          ...
          <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "Cairo", marginBottom: 4,
            color: players.find((p) => p.isHuman)?.correct ? theme.green : theme.red }}>
            {players.find((p) => p.isHuman)?.correct ? "إجابة صحيحة!" : "إجابة خاطئة!"}
          </div>
          <div style={{ color: theme.accent, fontSize: 18, fontWeight: 700, fontFamily: "Tajawal" }}>
            الجواب الصحيح: {question.answer ? "✅ صح" : "❌ خطأ"}
          </div>
        </Card>
      )}
