...
function MatchmakingScreen({ matchType, playerName, onBack, onHome, onPlayAI, onStartOnlineMatch }) {
  ...
  useEffect(() => {
    ...
    let myPid = null;
    (async () => {
      const pid = await joinLobby(playerName, matchType);
      if (!mounted) return;
      myPid = pid;
      setMyLobbyId(pid);
      ...
      const unsub = listenToLobby((data) => {
        ...
        const players = entries
          ...
          .map(([id, d]) => ({ id, name: d.name, matchType: d.matchType, status: d.status }));
          ...
          {lobbyPlayers.map((p) => (
            <Card key={p.id} onClick={() => handleSendInvite(p)} glow>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "Cairo", color: theme.text }}>{p.name}</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <Button
                onClick={() => {
                  ...
                  if (isOnlineGame && gameState?.roomCode && gameState?.myPlayerId) {
                    forfeitGame(gameState.roomCode, gameState.myPlayerId).catch(() => {});
                  }
                }}
                fullWidth
                color={theme.red}
