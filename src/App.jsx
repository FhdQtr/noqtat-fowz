...
function RoomScreen({ roomCode, joinCode, setJoinCode, matchType, playerName, isHost, roomData, onlineError, onJoinRoom, onBack, onHome, onStart }) {
  const [tab, setTab] = useState(isHost ? "create" : "join");
  const [copied, setCopied] = useState(false);
  const [joinError, setJoinError] = useState("");
  const [joining, setJoining] = useState(false);

  const needed = matchType === "2v2" ? 4 : 2;
  const playersJoined = roomData?.players ? Object.keys(roomData.players).length : (isHost ? 1 : 0);
  ...
  return (
    <div style={{ animation: "fadeIn 0.6s ease-out", paddingTop: 40 }}>
      ...
      {!isHost && !roomData && (
        <div style={{ display: "flex", gap: 8, marginBottom: 30, background: theme.bgCard, borderRadius: 14, padding: 4 }}>
          {["create", "join"].map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              style={{
                ...
                color: tab === t ? "#0a0a0f" : theme.textMuted,
                fontWeight: 700, fontSize: 16, fontFamily: "Cairo", cursor: "pointer", transition: "all 0.3s",
              }}
            >
              {t === "create" ? "أنشئ غرفة" : "ادخل غرفة"}
            </button>
          ))}
        </div>
      )}

      {(isHost || (roomData && tab === "create")) ? (
        <Card glow>
          <div style={{ textAlign: "center" }}>
            <p style={{ fontSize: 14, color: theme.textMuted, fontFamily: "Tajawal", marginBottom: 16 }}>
              شارك هالكود مع أصدقائك
            </p>
            <div
              style={{
                ...
                {Array.from({ length: needed }).map((_, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      {player && (
                        <span style={{ fontSize: 11, color: theme.text, fontFamily: "Cairo", fontWeight: 600 }}>
                          {player.name}
                        </span>
                      )}
                    </div>
            ...
            {isHost ? (
              ...
              <div style={{ marginTop: 20, padding: 12, background: `${theme.green}10`, borderRadius: 12, border: `1px solid ${theme.green}33` }}>
                <p style={{ color: theme.green, fontFamily: "Cairo", fontSize: 14, fontWeight: 700 }}>
        ...
        <Card glow>
          <div style={{ textAlign: "center" }}>
            ...
            />
            {joinError && (
              <p style={{ color: theme.red, fontSize: 13, marginBottom: 10, fontFamily: "Tajawal" }}>{joinError}</p>
            )}
            <Button
              onClick={handleJoin}
              fullWidth
