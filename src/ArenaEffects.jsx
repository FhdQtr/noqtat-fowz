      ...
      <div style={modalStyles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={modalStyles.tabs}>
          {tabs.map((tab) => {
            ...
            return (
              <button
                key={tab.id} onClick={() => setSelectedTab(tab.id)}
                style={{
                  ...modalStyles.tab,
                  ...(active ? { borderColor: tab.color, background: `${tab.color}15`, color: tab.color } : {}),
                }}
              >
                <span style={{ fontSize: 18 }}>{tab.icon}</span>
                <span style={{ fontSize: 11, fontWeight: 800, fontFamily: "Cairo" }}>{tab.label}</span>
                <span style={{
                  fontSize: 9, background: `${tab.color}30`, color: tab.color,
                  padding: "1px 6px", borderRadius: 999, fontWeight: 800,
                }}>
                  {groupedPowerups[tab.id].length}
                </span>
              </button>
            );
          })}
        ...
        <div style={{
          ...
          {groupedPowerups[selectedTab].map((p) => (
            <PowerupCard
              key={p.id} powerup={p}
...
function PowerupCard({ powerup, onBuy, onUse, purchasing }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 32, marginBottom: 6 }}>{powerup.icon}</div>
      <h3 style={{ fontSize: 13, fontWeight: 800, color: theme.text, margin: "0 0 4px", fontFamily: "Cairo" }}>
        {powerup.name}
      </h3>
      <p style={{
        fontSize: 10, color: theme.textMuted, margin: "0 0 10px",
        minHeight: 30, lineHeight: 1.4, fontFamily: "Tajawal",
      }}>
        {powerup.description}
      </p>
