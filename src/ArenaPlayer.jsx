...
function PlayingScreen({ arenaCode, playerId, arenaData }) {
      ...
      <div style={styles.screen}>
        <div style={{ ...styles.card, maxWidth: 540 }}>
          <div style={{ textAlign: "center", padding: "28px 16px 16px" }}>
            <div style={{ fontSize: 80, color: wasCorrect ? theme.green : theme.red, lineHeight: 1, marginBottom: 12 }}>
              {wasCorrect ? "✓" : "✗"}
            </div>
            <h2 style={{
              fontSize: 26, fontWeight: 800,
              color: wasCorrect ? theme.green : theme.red,
              margin: "0 0 12px", fontFamily: "Cairo",
            }}>
              {wasCorrect ? "إجابة صحيحة!" : selectedOption === null ? "انتهى الوقت" : "إجابة خاطئة"}
            </h2>
            {questionData && (
              <p style={{ fontSize: 14, color: theme.textMuted, margin: "0 0 14px", fontFamily: "Tajawal" }}>
            ...
            {myStreak > 1 && (
              <div style={{ fontSize: 14, color: theme.yellow, fontWeight: 700, fontFamily: "Cairo", marginTop: 8 }}>
          ...
          <RoundHeroes
            ...
            answers={arenaData?.answers?.[currentQ]}
  ...
  // مرحلة لوحة المتصدرين
  if (phase === "leaderboard") {
    const playersList = arenaData?.players
      ? Object.values(arenaData.players).sort((a, b) => (b.score || 0) - (a.score || 0))
      : [];

    return (
      <div style={styles.screen}>
        <div style={{ ...styles.card, maxWidth: 540 }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: theme.yellow, margin: "0 0 16px", fontFamily: "Cairo", textAlign: "center" }}>
            🏆 المتصدرون
          </h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
            {playersList.slice(0, 6).map((p, idx) => (
              <div key={p.name} style={{
                ...styles.playerItem,
                ...(idx === 0 ? { borderColor: theme.yellow, background: `${theme.yellow}15` } : {}),
                ...(idx === 1 ? { borderColor: "#9ca3af", background: "rgba(156,163,175,0.1)" } : {}),
                ...(idx === 2 ? { borderColor: "#d97706", background: "rgba(217,119,6,0.1)" } : {}),
              }}>
                <span style={{ fontSize: 22, minWidth: 36, textAlign: "center" }}>
                  {idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : `#${idx + 1}`}
                </span>
                <span style={{ flex: 1, fontSize: 15, fontWeight: 700, fontFamily: "Cairo" }}>{p.name}</span>
                <span style={{ fontSize: 17, fontWeight: 800, color: theme.yellow, fontFamily: "Cairo" }}>{p.score || 0}</span>
              </div>
            ))}
          </div>
          <p style={{ textAlign: "center", color: theme.textMuted, margin: 0, fontSize: 13, fontFamily: "Tajawal" }}>
            السؤال القادم خلال لحظات...
          </p>
        </div>
        <ArenaPlayerStyles />
      </div>
  ...
  return (
    <style>{`
      ...
      input:focus {
        ...
        box-shadow: 0 0 0 3px ${theme.yellow}22 !important;
      }
