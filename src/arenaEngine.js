...
const PHASE_DURATIONS = {
  ...
  reveal:      4,  // مدة كشف الإجابة الصحيحة
  leaderboard: 5,  // مدة عرض لوحة المتصدرين بين الأسئلة
...
export const POWERUPS_CATALOG = {
  ...
  jackpot: {
    ...
    name: "الجاكبوت",
    icon: "🎰",
    description: "×3 لكن خسارة مضاعفة لو غلط",
    cost: 80,
    type: "multiplier",
    requiresTarget: false,
  },
  ...
  poll: {
    ...
    name: "الاستفتاء",
    icon: "📊",
    description: "شوف توزيع إجابات الباقين (يتطلب ستريك)",
    cost: 90,
    type: "special",
    requiresTarget: false,
    streakRequired: 3,
...
export function calculateRoundScores(question, answers, players) {
  ...
  for (const [playerId, player] of Object.entries(players)) {
    ...
    if (isCorrect) {
      ...
      newStreak = 0;
      // الجاكبوت: خسارة مضاعفة لو فعّل قدرة الجاكبوت
      if (player.activeJackpot) {
        pointsGained = -basePoints; // خسارة بحجم نقاط السؤال
      }
    }
    ...
    updates[`players/${playerId}/activeJackpot`] = null;
...
export function useArenaEngine(arenaCode, isHost, questionBank) {
  ...
  async function handlePhaseEnd() {
    ...
    try {
      ...
      if (phase === "question") {
        ...
        await transitionToLeaderboard(data, currentQ);
  ...
  // ─── انتقال 2: من reveal إلى leaderboard ───────────────────────

  async function transitionToLeaderboard(data, currentQ) {
    await update(ref(db, `arenas/${arenaCode}`), {
      phase: "leaderboard",
      phaseStartedAt: Date.now(),
    });
  ...
  // ─── انتقال 3: للسؤال القادم أو إنهاء اللعبة ───────────────────
...
export function usePowerups(arenaCode, playerId, playerData) {
  ...
  async function activatePowerup(powerupId, targetPlayerId = null) {
    ...
    try {
      ...
      switch (powerupId) {
        case "multiplier":
          updates[`players/${playerId}/activeMultiplier`] = 2;
          break;
        case "jackpot":
          updates[`players/${playerId}/activeMultiplier`] = 3;
          updates[`players/${playerId}/activeJackpot`] = true;
          break;
        case "shield":
          updates[`players/${playerId}/activeShield`] = true;
          break;
        case "extraTime":
          updates[`players/${playerId}/extraTime`] = 10;
          break;
        case "ice":
          updates[`players/${targetPlayerId}/frozenForQuestion`] = true;
          break;
        case "mute":
          updates[`players/${targetPlayerId}/mutedFor`] = 3; // ثواني
          break;
        case "scramble":
          updates[`players/${targetPlayerId}/scrambled`] = true;
          break;
        case "speed":
          updates[`globalEffects/speedFor`] = { from: playerId, questions: 2 };
          break;
        case "steal":
          // التنفيذ في الملف 5 (يحتاج معرفة المتصدر)
          updates[`pendingActions/steal`] = { from: playerId, at: Date.now() };
          break;
        // ... باقي القدرات في الملف 5
      }
