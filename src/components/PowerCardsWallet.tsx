...
const CARD_COPY: Record<PowerCardId, { hint: string; moment: string; detail: string }> = {
  ...
  pickPlayer: { hint: "اختار لاعبًا واحدًا يجاوب", moment: "أثناء سؤال الفريق الخصم وقبل أن يجيب", detail: "تختارون لاعبًا واحدًا من الخصم ليجيب وحده بدل ممثل الفريق. يعمل عندما يكون في الفريق لاعبان أو أكثر، ولا يعمل في وضع «المقدم فقط»." },
...
function timingReason(match: Match, teamCode: string, card: PowerCardId): string | null {
  ...
  if (card === "pickPlayer") {
    ...
    if (match.answerMode === "host") return "لا يعمل هذا الكرت عندما يكون المقدم هو الذي يثبت الإجابة.";
