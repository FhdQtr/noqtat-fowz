...
export async function createRoom(roomCode, hostName, matchType) {
  ...
  await set(roomRef, {
    ...
    players: { [generatePlayerId()]: { name: hostName, ready: true, score: 0 } },
    createdAt: Date.now(),
  });
  const playerId = Object.keys((await get(roomRef)).val().players)[0];
...
export async function joinMatchmaking(playerName, matchType) {
  ...
  // Auto-cleanup on disconnect
  onDisconnect(ref(db, `matchmaking/${matchType}/${playerId}`)).remove();
